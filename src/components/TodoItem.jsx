...
const TodoItem = (props) => {
  ...
  const update = (id, value, e) => {
    if(e.which === 13){
      // here 13 is the key word for enter key
  ...
  return(
    ...
    whileHover={{ scale:.9, transition: {type: "spring", duration: 0.1} }}
    ...
    key={item.id} className='card'>
              ...
              <motion.button whileHover={{ scale: 1.4 }} whileTap={{ scale: 0.9 }}   onClick={() => changeFocus()}>{" "}<AiFillEdit />{" "}</motion.button>
              {
                item.completed === false && (
                  <motion.button whileHover={{ scale: 1.4 }} whileTap={{ scale: 0.9 }}  style={{color:"green"}} onClick={() => completeTodo(item.id)}>{" "}<IoCheckmarkDoneSharp />{" "}</motion.button > 
                )
              }
               <motion.button whileHover={{ scale: 1.4 }} whileTap={{ scale: 0.9 }} style={{color:"red"}} onClick={() =>removeTodo(item.id)}>{" "}
               <IoClose /> {" "}
                </motion.button>
              </div>
